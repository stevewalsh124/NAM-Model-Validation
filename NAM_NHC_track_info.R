<<<<<<< HEAD
library(dplyr)
track_files <- list.files("Downloads/NAM_NHC_track/NAMbesttrack2015/", full.names = T)
...
all_track_folders <- list.dirs("~/Downloads/NAM_NHC_track/")
for(f in 2:length(all_track_folders)){
  ...
  for (i in 1:length(list.files(all_track_folders[f], full.names = T))) {
    ...
    if("  NAM" %in% levels(track_file$V5)){NAM_files[i] <- 1}
    else{NAM_files[i] <- 0}
    
    #indicates if a file contained HU/TS info
    if(" HU" %in% levels(track_file$V11) | " TS" %in% levels(track_file$V11)){HU_TS_files[i] <- 1}
    else{HU_TS_files[i] <- 0}
    ...
    else{print("no lines with NAM and HUR info"); NAM_HU_files[i] <- 0}
  }
...
>>>>>>> d5885a2abb80d7a930734618038d728f43d6a116
...
most.storm.folders <- list.dirs("/Volumes/LACIEHD/NAMandST4", recursive = F)[44:47] #/Volumes/LACIEHD/
