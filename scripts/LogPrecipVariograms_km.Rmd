...
```{r plot.all.47}
pdf("~/NAM-Model-Validation/pdf/47storms_km.pdf")   #    ("LogPrecipVariograms_km_LCC_MaternSVGestimates.pdf") 

storm.dirs <- list.dirs("~/NAMandST4", recursive = F)#[-c(2,5,7,16,22,25,35,47)] #/Volumes/LACIEHD/
...
#[-c(2,7,27)] #skip these when fitting the directional variograms with LCC projn
...
NAM_km_for_mask <- projectRaster(raster("~/NAM-Model-Validation/nam_218_20050829_1200_f012.grib"),
                                 crs="+proj=lcc +lat_1=25 +lat_2=25 +lat_0=25 
                                 +lon_0=-95 +x_0=0 +y_0=0 +a=6371200 +b=6371200 
                                 +units=km +no_defs ")
mask <- projectRaster(mask, NAM_km_for_mask, method = 'ngb')
mask.regrid <- resample(mask, NAM_km_for_mask, method='ngb')
# plot(mask.regrid)
...
for(i in 1:length(storm.dirs)){
  ...
  for (k in 1:length(list.files(ST4_folder,full.names = T)[1:4])) {
    ...
    ST4list[[i]][[k]] <- (raster(ST4file) %>% projectRaster(crs="+proj=lcc +lat_1=25 +lat_2=25 +lat_0=25 +lon_0=-95 +x_0=0 +y_0=0 +a=6371200 +b=6371200 +units=km +no_defs "))
  ...
  for (j in 1:length(list.files(NAM_folder,full.names = T)[first24])) {
    ...
    print(NAM_band)
    NAMlist[[i]][[j]] <- (raster(NAMfile, band=NAM_band) %>% projectRaster(crs = "+proj=lcc +lat_1=25 +lat_2=25 +lat_0=25 +lon_0=-95 +x_0=0 +y_0=0 +a=6371200 +b=6371200 +units=km +no_defs"))
  ...
  hurdat_file <- list.files(storm.dirs[i], pattern = ".csv", recursive = F, full.names = T)
  hurdat <- read.csv(hurdat_file)
  colnames(hurdat) <- c("BTid", "SNum", "Year", "Month", "Day", "Hour", "Lat", "Lon", "Winds", "Pressure")
  hurdat[,8:7]<-project(as.matrix(hurdat[,8:7]), "+proj=lcc +lat_1=25 +lat_2=25 +lat_0=25 +lon_0=-95 +x_0=0 +y_0=0 +a=6371200 +b=6371200")/1000
  ...
  radius <- 700 * 1e2 #this became kilometers instead of degrees so I multiplied by 1e2
  ...
  # write.csv(NAM_df, paste0("csv/INLAvsWLS/NAM/namdf_",storm_year,storm_name,"_",radius,"km.csv"))
  
  ST4_spdf <- as(ST4_plotter, "SpatialPixelsDataFrame")
  ST4_df <- as.data.frame(ST4_spdf)
  colnames(ST4_df) <- c("value", "x", "y")
  # write.csv(ST4_df, paste0("csv/INLAvsWLS/ST4/st4df_",storm_year,storm_name,"_",radius,"km.csv"))
  ...
  # write.csv(error_df, paste0("csv/INLAvsWLS/error/errordf_",storm_year,storm_name,"_",radius,"km.csv"))
  ...
  writeRaster(error, paste0("~/NAM-Model-Validation/error_rasters_km/",storm_year,storm_name), overwrite=T)
  ...
  this_error_spdf <- as((this_error*mask), "SpatialPixelsDataFrame")
  ...
  plot.edge <-7.2 * 1e2  #this is approx the number of degrees (100km) from the storm's eye
  ...
  g1= ggplot(aes(x=x,y=y,fill=value),data=NAM_df) + 
    ...
    coord_fixed(xlim=c(-4294,3320),ylim=c(-904,4440), ratio = 1)
  ...
  # initial.values <- expand.grid(seq(0.01, 6, by=.02), seq(0.01, 1e3, by=10))
  # initial.values <- expand.grid(seq(0, 6, by=.01), seq(0, 4, by=.01))
  # initial.values <- c(2.96,1.34)
  # sim.mod.cressie     <- variofit(simul.modvar, ini.cov.pars=initial.values, cov.model="matern", 
                                  # fix.nugget=F, weights="cressie", fix.kappa = T, kappa = 1) # , nugget=0
  ...
  # kappavec[i] <- sim.mod.cressie$kappa
...
###################################################
#Sum up the error fields, also using moving windows
### ### ### ### ### ### ### ### ### ### ### ### ###


#break storms into 5 year moving windows... first is 2004-2008, last is 2013-2017
# window_subs <- list(1:28,8:29,16:29,19:32,23:36,29:37,30:38,30:40,33:43,37:47)
# raster_files <- list.files("error_rasters", pattern = ".grd", full.names = T)
# pdf("pdf/5storm_moving_windows.pdf")
# for (t in 1:(length(raster_files)-5)) {
#   error_sum <- mosaicList(raster_files[t:(t+5)])
#   # plot(error_sum)
...
#   
#   g4= ggplot(aes(x=x,y=y,fill=value),data=all_error_df) + 
#   geom_tile() + theme_classic() + 
#   geom_polygon(data=subset(map_data("state"), region %in% regions), 
#                aes(x=long, y=lat, group=group), colour="black", fill="white", alpha=0) +
#   scale_fill_gradient2(low = "blue",mid = "white", high = "red",na.value = "white") +
#   labs(title =paste("Storms",t," to ", t+5,"Sum of Errors: NAM - ST4"))+
#   coord_fixed(xlim=c(-110,-65),ylim=c(25,50), ratio = 1)  
#   plot(g4)
# }
# dev.off()

# pdf("pdf/5year_moving_windows.pdf")
# for (t in 1:10) { #10 because 2004to2008 - 2013to2017
#   error_sum <- mosaicList(raster_files[window_subs[[t]]])
#   # plot(error_sum)
#   all_error_spdf <- as((error_sum), "SpatialPixelsDataFrame")
#   all_error_df <- as.data.frame(all_error_spdf)
#   colnames(all_error_df) <- c("value", "x", "y")
...
#                aes(x=long, y=lat, group=group), colour="black", fill="white", alpha=0) +
#   scale_fill_gradient2(low = "blue",mid = "white", high = "red",na.value = "white") +
#   labs(title =paste("Years",2003+t," to ", 2007+t,"Sum of Errors: NAM - ST4"))+
#   coord_fixed(xlim=c(-110,-65),ylim=c(25,50), ratio = 1)  
#   plot(g4)
# }
# 
...
plot(errorcounts)
plot(errorcounts*mask)

# raster_files <- list.files("error_rasters", pattern = ".grd", full.names = T)
# error_sum <- mosaicList(raster_files)

#bad right now, figure out mismatched extents
# avg_errors <- error_sum/resample(errorcounts*mask,error_sum)
# plot(avg_errors)

# svg.param.ests.error.km <- cbind(all.storm.names,all.storm.years,phivec,prRangevec,tau2vec,sig2vec, kappavec)
# write.csv(svg.param.ests.error, "csv/INLAvsWLS/svg.param.ests.error_KM_LCC_MaternSVGestimates.csv")
# print(svg.param.ests.error.km)
