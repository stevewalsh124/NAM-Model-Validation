...
lambda_files <- list.files("~/NAM-Model-Validation/csv/myMLEresults/myMLEs/", 
                          pattern = "[0-9].csv", full.names = T)[1:47]
lambda_hat <- do.call(rbind, lapply(lambda_files, read.csv, row.names = 1))[,1:2]
theta_hat <- cbind(log(lambda_hat[,2]/lambda_hat[,1]), log(lambda_hat[,2]))
# load(file = paste0("~/NAM-Model-Validation/RData/Gibbs_sqrt_LM4",
...
pdf(paste0("~/NAM-Model-Validation/pdf/prediction/prediction_sqrt_",name,
           year,"_GIS_GHiG_NA_flatPWmean_",PWstamp, Ngen,"_LM4.pdf"))
...
# emp_mu_theta <- colMeans(mu_burn)
# Sigma_theta_pred <- as.square(apply(Sigma_burn, 2, median))
# theta_pred <- rmvn(Ngen, emp_mu_theta, Sigma_theta_pred)

B <- 1000
theta_pred <- matrix(NA, B, 2)
for (b in 1:B) { theta_pred[b,] <- theta_hat[sample(1:nrow(theta_hat),1),] }
...
write.csv(ests, paste0(file = "~/NAM-Model-Validation/csv/prediction/", pred_dir,"_sqrt_LM4_",
                       PWstamp,".csv"))
...
save.image(paste0("~/NAM-Model-Validation/RData/prediction/prediction",ste,"_LM4_",PWstamp,".RData"))
dev.off()
