...
pdf("~/NAM-Model-Validation/pdf/Gibbs/GibbsSamplerSimsRegr_empBayes.pdf")
# remove(list=ls())
...
theta_hat <- as.matrix(estvals)#matrix(NA, nrow= length(par_optim[[1]]), ncol=length(par_optim[[1]][[1]]))
N <- nrow(theta_hat)
P <- ncol(theta_hat)
# for(i in 1:N){ theta_hat[i,] <- par_optim[[1]][[i]] }
# colnames(theta_hat) <- c("MLEsigma2","MLEphi","MLEkappa") 

hessians <- hess_sim#par_optim[[2]]
...
for (i in 1:length(hessians)) {
  ...
  if(!isSymmetric.matrix(hessians[[i]])){
...
true_M<- emp_B#sum_th_xt_true %*% round(solve(sum_x_xt), rounder)
...
S0 <- cov(theta_hat)#diag(c(.01,.01,.01))                #EMPIRICAL BAYES
...
for (i in 2:iters) {
  ...
  # Sigma_theta[i,] <- symmtrz(as.square(riwish(N + v0, L + S0)))
...
apply(log(theta_i_sim) < emp_thetaUB & log(theta_i_sim) > emp_thetaLB, 2, sum)/N
...
hess_vec <- cov_vec <- matrix(NA, N, P^2)
for (i in 1:N) {
  hess_inv[i,] <- sqrt(diag(solve(hessians[[i]])))
  hess_vec[i,] <- c(hessians[[i]])
  cov_vec[i,] <- c(solve(hessians[[i]]))
}
...
# save.image(file = "~/NAM-Model-Validation/RData/Gibbs_sims.RData")

print("posterior median, LB and UB for matrix B"); emp_B; emp_B_LB; emp_B_UB
print("posterior median, LB and UB for Sigma_theta"); emp_Sigma_theta; emp_Sigma_theta_LB; emp_Sigma_theta_UB

#load what the true generating emp_B and emp_Sigma_theta were
load("~/NAM-Model-Validation/RData/emp_B")
load("~/NAM-Model-Validation/RData/emp_Sigma_theta")

print("see if true B is in credible interval"); emp_B >= emp_B_LB & emp_B <= emp_B_UB
print("truth, LB and UB for matrix B"); emp_B; emp_B_LB; emp_B_UB

print("see if true Sigma_theta is in credible interval"); emp_Sigma_theta >= emp_Sigma_theta_LB & emp_Sigma_theta <= emp_Sigma_theta_UB
print("truth, LB and UB for Sigma_theta"); emp_Sigma_theta; emp_Sigma_theta_LB; emp_Sigma_theta_UB




# figuring out what the actual true B and sigma_theta are...
cov(theta_i_sim[which(loc_int$loc=="ATL"),])
cov(theta_i_sim[which(loc_int$loc=="FL"),])
cov(theta_i_sim[which(loc_int$loc=="GULF"),])

samp <- rmvn(10000, mu = theta_i_sim[1,], Sigma=as.square(emp_Sigma_theta))
colMeans(samp)
cov(samp)
