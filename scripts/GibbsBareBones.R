...
for (nr in 1:nreps) {
  ...
  par(mfrow=c(1,3))
  for (i in 1:P) {
    plot(theta_i_sim[,i], theta_hat_sim[,i], main=paste("sim true vs sim hat", i))
    abline(0,1)
  }
  ...
  for (i in 2:iters) {
    ...
    for(j in 1:N){
      ...
      Etheta_i <- Vtheta_i %*% ((GHGt[[j]]%*%theta_hat[j,]) + 
  ...
  hat_cover[nr,] <- colMeans((theta_hat_sim) < emp_thetaUB & (theta_hat_sim) > emp_thetaLB)
  ...
  for (j in bad_thetas) {
    for(i in 1: P) {
      ...
      abline(h=theta_hat_sim[j,i], lwd=2, col="green3")
    }
  }
