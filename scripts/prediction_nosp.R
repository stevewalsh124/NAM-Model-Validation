## Prediction for no spatial dependence case:
## Based on predictionNA_2018_SE.R
...
for (ste in 1:length(pred_dirs)) {
  ...
  if(straw){
    ...
    for (i in 1:Ngen) {
      theta_pred[i,] <- rmvn(1, t(matrix(B_burn[10*i,], P, R) %*% x_pred), Sigma_burn[10*i])
    }
  }
  ...
  simvals <- matrix(NA, nrow = nrow(NAM_pred), ncol = Ngen) # simPWM <-
  for (g in 1:Ngen) {
    if(g %% 1000 == 0) cat(g,"\n")
