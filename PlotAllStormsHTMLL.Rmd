...
Plotting 44 Storms' Precipitation for First 24 Hours After Landfall
...
for(i in 1:length(all.storm.folders)){
  ...
  brk=seq(-1*ceiling(error.max/5)*5,ceiling(error.max/5)*5,ceiling(error.max/5))
  my.limits = c(-1*ceiling(error.max/5)*5,ceiling(error.max/5)*5)
  ...
  g3= ggplot(aes(x=x,y=y,fill=value),data=error_df) + 
    ...
    geom_polygon(data=subset(map_data("state"), region %in% regions), 
                 aes(x=long, y=lat, group=group), colour="black", fill="white", alpha=0) +
    scale_fill_gradient2(low = "blue",mid = "white", high = "red",na.value = "white",limits=my.limits) +
