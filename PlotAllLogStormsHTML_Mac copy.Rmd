...
#storms that abort R: c(5,16,22,25,35,47) 
#storms w/o 12 hr precip: c(2,3,6,8)
storms.out.of.hurdat <- c() #storm #s 5,31,35,42,48
# > all.storm.folders[storms.out.of.hurdat]
# [1] "NAMandST4/2004hermine" "NAMandST4/2004hermine" "NAMandST4/2011don"     "NAMandST4/2012debby"  
# [5] "NAMandST4/2016colin"   "NAMandST4/2017nate" 
...
mask.regrid <- resample(mask, 
                        projectRaster(raster("nam_218_20050829_1200_f012.grib"),
...
for(i in 1:length(most.storm.folders)){
  # i <- 1
  currentST4_NAM_folders <- list.dirs(most.storm.folders[i])[-1] #uncomment [-1] when st4&nam dirs together
  ...
  for (j in 1:length(list.files(NAM_folder,full.names = T)[first24])) {
    ...
    # if(storm_name=="matthew"&&j==1&&storm_date<20050000){NAM_band <- 14} 
    # if(storm_name=="matthew"&&j==2&&storm_date<20050000){NAM_band <- 12} 
  ...
  simul <- cbind(error_df[,2],error_df[,3],error_df[,1]) #uses degrees: Long, Lat, Value
  # write.csv(x = simul, 
  #           file = paste0("C:/Users/thefl/Downloads/NAM-Model-Validation/storm_df_deg/",
  #                         storm_date,storm_name,".csv"))
  # simul <- cbind(latlong2grid(simul[,1:2]), simul[,3]) #use km instead of degrees
  # write.csv(x = simul, 
  #           file = paste0("C:/Users/thefl/Downloads/NAM-Model-Validation/storm_df_km/",
  #                         storm_date,storm_name,".csv"))
  simul.geo <- as.geodata(simul)

  simul.modvar <- variog(simul.geo,estimator.type="modulus")

  initial.values <- expand.grid(c(seq(.01,.09,.01),seq(0.1, 6, by=.02),seq(6,20,1)), #sig2 (partial sill)
                                100*c(seq(.01,.09,.01),seq(0.1, 4, by=.02),seq(4,20,1))) #phi (range)
  # initial.values <- c(2.96,1.34)
  # sim.mod.cressie     <- variofit(simul.modvar, ini.cov.pars=initial.values, cov.model="matern",
                                  # fix.nugget=TRUE, nugget=0, weights="cressie", fix.kappa = F)
  sim.mod.cressie.nug <- variofit(simul.modvar, ini.cov.pars=initial.values, cov.model="gaussian",
                                  fix.nugget=F             , weights="cressie", fix.kappa = F)

  # SVGparamfix[[i]] <- sim.mod.cressie
  SVGparamest[[i]] <- sim.mod.cressie.nug
  print(sim.mod.cressie.nug)
  tau2vec[i] <- sim.mod.cressie.nug$nugget
  sig2vec[i] <- sim.mod.cressie.nug$cov.pars[1]
  phivec[i]  <- sim.mod.cressie.nug$cov.pars[2]
  kappavec[i]<- sim.mod.cressie.nug$kappa 
  prRangevec[i]<-sim.mod.cressie.nug$practicalRange
  
  vario.plot <- plot(simul.modvar,main=paste0("SVG for ", storm_year," ",storm_name),
                     ylim=c(0,1));lines(sim.mod.cressie.nug,col="red")
  # lines(sim.mod.cressie,col="orange");
...
landfalls = read.csv(textConnection(skip_second), header = TRUE, stringsAsFactors = FALSE)
