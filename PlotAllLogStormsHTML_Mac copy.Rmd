...
```{r plot.all.41}
pdf("logbuffprecip.SVGbombskmphiquest.pdf")
all.storm.folders  <- list.dirs("NAMandST4", recursive = F) #/Volumes/LACIEHD/
most.storm.folders <- list.dirs("NAMandST4", recursive = F)[1:3]#[c(5,16,22,25,35,47)] #Matern aborters
...
for(i in 1:length(most.storm.folders)){
  ...
  # simul[,1:2] <- latlong2grid(simul[,1:2])
  simul <- cbind(latlong2grid(simul[,1:2]), simul[,3])
  simul.geo <- as.geodata(simul)

  simul.modvar <- variog(simul.geo,estimator.type="modulus")

  initial.values <- expand.grid(c(seq(.01,.09,.01),seq(0.1, 6, by=.02),seq(6,20,1)),
                                100*c(seq(.01,.09,.01),seq(0.1, 4, by=.02),seq(4,20,1)))
...
pairs(cbind(tau2vec,phivec,sig2vec,kappavec))
```