...
```{r plot.all.41}
# pdf("logbuffer.prevariogram4.pdf")
all.storm.folders <- list.dirs("/Volumes/LACIEHD/NAMandST4", recursive = F) #
most.storm.folders <- list.dirs("/Volumes/LACIEHD/NAMandST4", recursive = F)[42]#[-c(1,12:36)] #
...
mask.regrid <- resample(mask, projectRaster(raster("/Volumes/LACIEHD/nam_218_20050829_1200_f012.grib"),
...
for(i in 1:length(most.storm.folders)){
  ...
  if(length(list.files(NAM_folder,full.names = T))==18){first24 <- c(7,8)} else {
    if(length(list.files(NAM_folder,full.names = T))==15|storm_name=="rita"){first24 <- c(5,6)} #rita is missing files
  ...
  for (j in 1:length(list.files(NAM_folder,full.names = T)[first24])) {
    ...
    if(storm_name=="ivan"   &&j==2&&storm_date<20050000){NAM_band <- 14} 
    if(storm_name=="matthew"&&j==1&&storm_date<20050000){NAM_band <- 13} 
    if(storm_name=="matthew"&&j==2&&storm_date<20050000){NAM_band <- 13} 
    if(storm_name=="frances"&&j==1&&storm_date<20050000){NAM_band <- 13} 
    if(storm_name=="frances"&&j==2&&storm_date<20050000){NAM_band <- 13} 
  ...
  initial.values <- expand.grid(seq(0.1, 6, by=.02), seq(0.1, 4, by=.02))
  ...
  sim.mod.cressie     <- variofit(simul.modvar, ini.cov.pars=initial.values, cov.model="gaussian", 
                                  fix.nugget=F, weights="cressie", fix.kappa = F)
  # , nugget=0
  # sim.mod.cressie.nug <- variofit(simul.modvar, ini.cov.pars=initial.values, cov.model="matern", 
                                  # fix.nugget=F             , weights="cressie", fix.kappa = F)
  
  sim.mod.cressie$nugget
  sim.mod.cressie$cov.pars
  sim.mod.cressie$kappa
  sim.mod.cressie$practicalRange
  
  # sim.mod.cressie.nug$nugget
  # sim.mod.cressie.nug$cov.pars
  # sim.mod.cressie.nug$kappa
  # sim.mod.cressie.nug$practicalRange
  #orange=red right now
